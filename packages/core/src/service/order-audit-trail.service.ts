...
export class OrderAuditTrailService {
  ...
  async logOrderStatusChanged(
    ...
    await this.pushChanges(
      orderId,
      OrderAuditTrailType.STATUS,
      newStatus,
      oldStatus,
    );
  }

  async logOrderLocationChanged(
    orderId: string,
    newLocation: string,
    oldLocation?: string,
  ) {
  ...
  async logOrderNotified(orderId: string) {
    await this.pushChanges(
      orderId,
      OrderAuditTrailType.NOTIFICATION,
      true,
      undefined,
    );
  }

  async logOrderPayment(
    orderId: string,
    newAmount: number,
    oldAmount?: number,
  ) {
  ...
  async getEntriesForDayWithoutDataType(
    date: Date,
    ...
    const dtos = await this.auditRepository.getOrderAuditTrailEntriesBetweenTs(
      startTs,
      endTs,
    );
    const filteredDtos = dtos.filter(
      (dto) => dto.type !== OrderAuditTrailType.DATA.toString(),
    );
  ...
  private async pushChanges(
    ...
    if (type === OrderAuditTrailType.DATA) {
      ...
      await this.auditRepository.createOrderAuditTrailEntry(
        OrderAuditTrailService.toDto(entry),
      );
    }
  }
  ...
  private static toDtoDataEdit(
    entry: OrderAuditTrailDataEditEntry,
