...
export class CustomerService {
  ...
  public async getAllCustomersMap(
    filterIds: string[],
    ...
    const dtos = (
      await Promise.all(
        [...filterSet].map((id) => this.repository.getCustomerById(id)),
      )
  ...
  public async getAllCustomersPaginated(
    ...
    const paginatedDtos =
      await this.repository.getAllCustomersPaginated(nextKey);

    return {
      customers: paginatedDtos.elements.map((dto) =>
        CustomerService.fromDto(dto),
      ),
