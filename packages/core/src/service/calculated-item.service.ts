...
export class CalculatedItemService {
  ...
  public async getCalculatedItem(
    orderId: string,
  ): Promise<CalculatedItem | null> {
    const dto =
      await this.calculatedItemRepository.getCalculatedItemById(orderId);
  ...
  public async createCalculatedItem(
    ...
    const partPromises: Promise<CalculatedItemPart>[] =
      item.partsToCalculate.map((p) => this.calculatePart(p, orderDimensions));
  ...
  public async saveCalculatedItem(
