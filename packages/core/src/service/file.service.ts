...
export class FileService {
  ...
  public async storeOptimizations(
    ...
    if (fileDto.optimizedKey == null) {
      ...
      await S3Util.uploadToS3(
        ...
        optimizedImage,
        optimizationAndThumbnailTypeInfo?.optimizedContentType ??
    ...
    if (fileDto.thumbnailKey == null) {
      ...
      await S3Util.uploadToS3(
        ...
        thumbnailImage,
        optimizationAndThumbnailTypeInfo?.thumbnailContentType ??
          originalFile.contentType,
      );
    }
  ...
  private static getAllFileKeys(fileDto: FileDto): string[] {
    return [fileDto.key, fileDto.optimizedKey, fileDto.thumbnailKey].filter(
      (key) => key != null,
    );
  }
}
