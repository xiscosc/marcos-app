...
export class PricingService {
  ...
  public async getPriceListByInternalId(
    id: string,
  ): Promise<ListPrice | undefined> {
  ...
  public async batchStoreListPrices(
    ...
    await this.listPricingRepository.batchStoreListPrices(
      ...
      prices
        ...
        .map((price) =>
          PricingService.toDto(PricingService.cleanAndVerifyEntity(price)),
        ),
    );
  }
  ...
  public async getPriceFromListById(
    pricingType: PricingType,
    id: string,
  ): Promise<ListPrice> {
    const priceDto = await this.listPricingRepository.getByTypeAndId(
      pricingType,
      id,
    );
    if (priceDto == null) {
  ...
  private async getFabricPriceList(
    ...
    const { d1, d2 } =
      PricingService.cleanAndOrderTotalDimensions(orderDimensions);
    const moldPrice = await this.getPriceFromListById(
      PricingType.MOLD,
      moldFabricId,
    );
  ...
  private static getMoldPrice(
    priceInfo: ListPrice,
    orderDimensions: OrderDimensions,
  ): number {
    const { d1, d2 } =
      PricingService.cleanAndOrderWorkingDimensions(orderDimensions);
    return getMoldPrice(priceInfo.price, d1, d2);
  }

  private static getFabricPrice(
    priceInfo: ListPrice,
    orderDimensions: OrderDimensions,
  ): number {
    const { d1, d2 } =
  ...
  private static getDimensionsByType(
    ...
    switch (price.type) {
      ...
      case PricingType.FABRIC:
        return PricingService.cleanAndOrderWorkingDimensions(orderDimensions);
      default:
        return PricingService.getDimensionsByFormula(
          price.formula,
          orderDimensions,
        );
    }
  ...
  private static getPriceByFormula(
    priceInfo: ListPrice,
    orderDimensions: OrderDimensions,
  ): number {
    const { d1, d2 } = PricingService.getDimensionsByFormula(
      priceInfo.formula,
      orderDimensions,
    );
  ...
  private static cleanAndVerifyEntity(listPrice: ListPrice): ListPrice {
    ...
    if (
      ...
      (listPrice.price == null ||
        (listPrice.price < 0 && listPrice.type !== PricingType.MOLD))
    ) {
  ...
  private static cleanAndOrderWorkingDimensions(
    orderDimensions: OrderDimensions,
  ) {
  ...
  private static cleanAndOrderTotalDimensions(
    orderDimensions: OrderDimensions,
  ) {
    return PricingService.cleanAndOrder(
  ...
  private static checkMaxMinDimensions(
    orderDimensions: OrderDimensions,
    pricing: ListPrice,
  ) {
    if (
      ...
      return;
    const { d1: d1w, d2: d2w } = PricingService.getDimensionsByType(
      pricing,
      orderDimensions,
    );
    if (pricing.maxD1 != null && pricing.maxD2 != null) {
      const { d1, d2 } = PricingService.cleanAndOrder(
        pricing.maxD1,
        pricing.maxD2,
      );
      if (d1w > d1 || d2w > d2)
