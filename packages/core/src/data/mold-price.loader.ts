...
export class MoldPriceLoader {
  ...
  public async loadMoldPrices(fileName: string): Promise<void> {
    ...
    await this.service.batchStoreListPrices(
      PricingType.MOLD,
      Array.from(newPrices.values()),
    );
  }

  private async getPricesFromExcel(
    fileName: string,
  ): Promise<Map<string, ListPrice>> {
