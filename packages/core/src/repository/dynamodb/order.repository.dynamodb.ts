...
export class OrderRepositoryDynamoDb extends DynamoRepository<OrderDto> {
  ...
  public async getOrderByShortId(shortId: string): Promise<OrderDto | null> {
    const dtos = await this.getByIndex(
      OrderDynamoDbIndex.shortIdIndex,
      shortId,
    );
  ...
  public async updateAmountPayed(order: OrderDto) {
    ...
    await this.updateFields(
      order.uuid,
      new Map([["amountPayed", order.amountPayed]]),
    );
  }
  ...
  public async updateCustomerId(order: OrderDto) {
    ...
    await this.updateFields(
      order.uuid,
      new Map([["customerUuid", order.customerUuid]]),
    );
  }
  ...
  private filterByStoreIdAndDeleted(
    ...
    if (Array.isArray(dto)) {
      return dto.filter(
        (d) => d.storeId === this.config.storeId && d.status !== "deleted",
      );
    }

    return dto.storeId === this.config.storeId && dto.status !== "deleted"
      ? dto
      : null;
  }
