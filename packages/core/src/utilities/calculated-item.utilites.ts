...
export class CalculatedItemUtilities {
  public static getOrderDimensions(
    ...
    const workingWidth =
      CalculatedItemUtilities.roundUpToNearestGreaterFiveOrTen(totalWidth);
    const workingHeight =
      CalculatedItemUtilities.roundUpToNearestGreaterFiveOrTen(totalHeight);
    return {
      workingWidth,
      workingHeight,
      totalHeight,
  ...
  public static sortByPricingType(
    ...
    return arr.sort((a, b) => {
      ...
      const aIndex =
        aValue !== undefined
          ? desiredOrder.indexOf(aValue)
          : desiredOrder.length;
      const bIndex =
        bValue !== undefined
          ? desiredOrder.indexOf(bValue)
          : desiredOrder.length;
